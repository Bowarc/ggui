...
pub struct UiManager {
    ...
    render_request: crate::render::RenderRequest,
}
...
impl UiManager {
    ...
    pub fn draw(
        ...
        canvas.draw(
            &ggez::graphics::Mesh::from_data(ctx, background_mesh.build()),
            ggez::graphics::DrawParam::default(),
        );

        canvas.draw(
            &ggez::graphics::Mesh::from_data(ctx, ui_mesh.build()),
            ggez::graphics::DrawParam::default(),
        );

        let render_log = crate::render::render(ctx, render_request, &mut canvas).unwrap();
        println!("{render_log:?}");

        canvas.draw(
            &ggez::graphics::Mesh::from_data(ctx, foreground_mesh.build()),
            ggez::graphics::DrawParam::default(),
        );
